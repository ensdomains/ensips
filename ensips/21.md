...
ensip:
    created: "2025-03-15"
    status: draft
ignoredRules: ["heading:annex:-fault-tolerance", "heading:annex:-tunneling"]
---
...
To perform an `OffchainLookup` on a website which is using [Content Security Policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/CSP) (CSP), implement the following logic:
* Add a trusted [external](#external-batch-gateways) batch gateway(s) to the CSP header
* Redirect `OffchainLookup` reverts with urls that contain `x-batch-gateway:true` directly to the external batch gateways
* Redirect all other `OffchainLookup` reverts to the external batch gateway, wrapped as a batch of one (and unwrapped on response)
...
[$EXTERNAL_GATEWAYS](./21/gateways.md)
