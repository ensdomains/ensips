...
This ENSIP allows third-party indexing services to listen to the `MetadataChanged` event to automatically discover and index metadata events from different chains and smart contracts, without relying on centralised RPC endpoint repositories.
...
- `rpcUrl` is optional. 
- `baseRegistry` is optional. If this address is specified, events will be emitted from this address. The interface of this contract is yet to be determined.
...
The MetadataChanged event emits the same information `metadata` function returns so that indexers cansubscribe to the event as the detail changes than periodically querying the function.
