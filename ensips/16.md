...
JSON RPC endoint must implement the following Solidity events. The endpoints must encode these events following [EIP-1474 eth_getLogs](https://eips.ethereum.org/EIPS/eip-1474#eth_getlogs) standard.
...
NOTE: Even though ENS v2 registry contract is ERC1155 (TransferSingle, TransferBatch), ERC721 events(Transfer) are also accepted if name owner choose to build their own registry with NFT capability.
