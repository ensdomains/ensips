...
While not mandatory, indexers are STRONGLY RECOMMENDED to provide GraphQL APIs for application-facing queries. GraphQL offers significant advantages for ENS applications that need to query name data:

**Query Flexibility**: Applications often need to make a wide variety of queries with different data requirements. GraphQL allows applications to request exactly the data they need in a single query, reducing round trips and avoiding over-fetching or under-fetching data.

**Unpredictable Query Patterns**: It is difficult to impossible for indexers to predict the full set of queries applications may need to make. REST APIs often result in redundant and repeated queries to gather all required data, while GraphQL enables complex data retrieval in a single request.

**Schema Documentation**: GraphQL's introspection capabilities provide self-documenting APIs, making it easier for developers to discover available queries and data structures.

**Ecosystem Compatibility**: Many existing ENS tools and applications are built to consume GraphQL APIs (particularly those compatible with The Graph subgraph schemas), improving adoption and developer experience.

### Implementation Guidance

Indexers implementing GraphQL APIs should:

**Consider Backwards Compatibility**: Where feasible, maintain backwards compatibility with existing ENS subgraph GraphQL schemas to support community tools and applications. Document any schema differences clearly.

**Support Both Single-Name and Cross-Name Queries**: Expose queries for both individual name lookups (e.g., "get all records for name X") and cross-name data (e.g., "find all names owned by address Y").

**Document Schema Versions**: Clearly version and document GraphQL schemas, especially when making breaking changes or introducing new features.
