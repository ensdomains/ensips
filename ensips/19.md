...
5. `name` is the primary name if and only if it is [normalized](./15.md) and resolves to the same `addressBytes`.

#### Forward Resolution
6. Compute `node = namehash(name)` without applying [normalization](./15.md).
