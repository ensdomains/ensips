...
ensip:
  status: final
  created: 2023-03-14
ignoredRules: ["heading:supported-chains"]
---
...
5. If `name` is [unnormalized](./15.md), eg. `normalize("Nick.eth") != "Nick.eth"`:
   * Stop and display the address.
6. `name` is the primary name if and only if it [resolves](#forward-resolution) to the same `addressBytes`.

#### Forward Resolution
7. Compute `node = namehash(name)`.
8. Resolve `resolvedAddress = resolve(name, callData)` where:
   | `coinType` | `callData`
   | --:        | :--
   | `60`       | `abi.encodeCall(IAddrResolver.addr, (node))`
   | `*`        | `abi.encodeCall(IAddressResolver.addr, (node, coinType))`
9. If `resolvedAddress != addressBytes`, no primary name exists for this address.
   * Stop and display the address.
10. `name` is the primary name.
