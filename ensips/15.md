...
* [`spec.json`](../assets/15//spec.json) contains all [necessary data](#description-of-specjson) for normalization.
* [`nf.json`](../assets/15//nf.json) contains all [necessary data](#description-of-nfjson) for [Unicode Normalization Forms](https://unicode.org/reports/tr15/) NFC and NFD.
...
* An [Unicode emoji](https://www.unicode.org/reports/tr51/) is a [single entity](https://unicode.org/reports/tr29/#Grapheme_Cluster_Boundaries) composed of one or more codepoints:
	...
	* [`spec.json`](#description-of-specjson) contains the complete [list of valid](../assets/15//emoji.md) **Emoji Sequences**.		
...
Convert a label into a list of `Text` and `Emoji` tokens, each with a payload of codepoints.  The complete list of character types and [emoji sequences](../assets/15//emoji.md#valid-emoji-sequences) can be found in [`spec.json`](#description-of-specjson).  
...
* **Groups** (`"groups"`) — [groups](../assets/15//groups.md) of characters that can constitute a label
...
* **Ignored** (`"ignored"`) — [characters](../assets/15//ignored.csv) that are ignored during normalization
...
* **Fenced** (`"fenced"`) — [characters](../assets/15//fenced.csv) that cannot be first, last, or contiguous
	* Example: `2044 (⁄) FRACTION SLASH`
* **Emoji Sequence(s)** (`"emoji"`) — valid [emoji sequences](../assets/15//emoji.md#valid-emoji-sequences)
	* Example: `👨‍💻 [1F468 200D 1F4BB] man technologist`
* **Combining Marks / CM** (`"cm"`) — [characters](../assets/15//cm.csv) that are [Combining Marks](https://unicode.org/faq/char_combmark.html)
* **Non-spacing Marks / NSM** (`"nsm"`) — valid [subset](../assets/15//nsm.csv) of **CM** with general category (`"Mn"` or `"Me"`)
* **Maximum NSM** (`"nsm_max"`) — maximum sequence length of unique **NSM**
* **Should Escape** (`"escape"`) — [characters](../assets/15//escape.csv) that shouldn't be printed
* **NFC Check** (`"nfc_check"`) — valid [subset](../assets/15//nfc_check.csv) of characters that [may require NFC](https://unicode.org/reports/tr15/#NFC_QC_Optimization)
...
* [Many characters](../assets/15//disallowed.csv) are **disallowed** for various reasons:
...
* **Groups** may contain [multiple scripts](../assets/15//groups.md):
...
* Emoji:
	...
	* Digits (`0-9`) are [not emoji](../assets/15//emoji.md#demoted-unchanged).
	* Emoji [mapped to non-emoji by IDNA](../assets/15//emoji.md#demoted-mapped) cannot be used as emoji.
	* Emoji [disallowed by IDNA](../assets/15//emoji.md#disabled-emoji-characters) with default text-presentation are **disabled**:
...
* Only [valid emoji sequences](../assets/15//emoji.md#valid-emoji-sequences) are permitted.
...
* [Supported Groups](../assets/15//groups.md)
* [Supported Emoji](../assets/15//emoji.md)
* [Additional Disallowed Characters](../assets/15//disallowed.csv)
* [**Ignored** Characters](../assets/15//ignored.csv)
* [**Should Escape** Characters ](../assets/15//ignored.csv)

## Appendix: Validation Tests

A list of [validation tests](../assets/15//tests.json) are provided with the following interpretation:
